...
var encry2arr_from = function(t, e, r) {  // 1  39  50  66
...
function o_default() {  // 71  87  89  91
    return 2 == arguments.length ? digest(update(new o_default(arguments[1]), arguments[0])) :
...
// todo 看逻辑，把二进制转换为字符串，parser过程
function Bt(t) {
    var dataBuf = Buffer.from(t)
    var e = {}
    e.maxObjectSize = 1e8
    function r(e) {
        var r = x[e]
        ...
        switch (a.objType) {
            case 0:
                return parseNullOrBool(a);
            case 1:
                return parseIntAndSign(a);
            case 2:
                return parseFloat(a);
            case 3:
                return parseDate(a);
    ...
    function parseNullOrBool(item) {
        switch (item.objInfo) {
            ...
            case 15:
                return null;
            default:
                throw new Error(3, item.objType.toString(16))
        }
    }
    function parseIntAndSign(item) {
        var len = Math.pow(2, item.objInfo); // 字节长度
        var start = item.offset + 1
        var end = start + len

        if (len > 4) {
            return u_h_Bt(a_slice(dataBuf, start, end))
        }

        if (len < e.maxObjectSize) {
            return dataBuf.subarray(start, end).readIntBE(0, len)
        }

        throw new Error("4 " + len + " " + e.maxObjectSize)
    }
    function p(item) { // 暂时没有执行
        var start = item.offset + 1
        var end = start + item.objInfo
        if (item.objInfo < e.maxObjectSize)
            return o({}, l, i(t.slice(start, end)));
        throw new Error("4 " + item.objInfo + " " + e.maxObjectSize)
    }
    function parseFloat(item) {
        var start = item.offset + 1
        var len = Math.pow(2, item.objInfo) // 计算字节长度
        var end = start + len
        if (len >= e.maxObjectSize)
            throw new Error("4 " + len + " " + e.maxObjectSize);
        var a = dataBuf.slice(start, end);
    ...
    function parseDate(item) {
        ...
        if (item.objInfo !== 3) {
            console.error(5, item.objInfo)
        }
        var seconds = dataBuf.subarray(start, end).readDoubleBE(0)
        return new Date(9783072e5 + 1e3 * seconds)
    }
    function y(r) {
        var n = r.offset
            , o = r.objInfo
...
function a_h_Bt(t, e = 0, r = t.length - e) {
    return readIntBE.call(t, e, r)
}
...
function r_r(r) {
    return e_e_decrypt()(e_e_decrypt_n)(r)
}


function e_e_decrypt() {
    return function(e) {
        return function(t) {
            const r = Bt(t)
            // console.log(4, Buffer.from(t).toString('hex'))
            return e(r)
        }
    }
}
...
function decrypt(r){
    ...
    var s = Math.max(Math.floor((a.length - 2 * i) / 3), 0)  // 40
    var u = a_slice(a,s, s + i);  // 41
    
    var aa =  concat([a_slice(a, 0, s), a_slice(a,s + i)]);  // 43  45  47
...
var keyLen = 16
async function dec() {
    var buf = Base64Binary.decode(r)
    var s = Math.floor((buf.length - 2 * keyLen) / 3)
    var keyBuf = buf.subarray(s, s + keyLen)
    var newBuf = new Uint8Array([...buf.subarray(0, s), ...buf.subarray(s + keyLen)])
    var seed = 41405
    var key = XXHash.h64(Buffer.from(keyBuf), seed).toString(16)
    var d = rc4Decrypt(newBuf, key)
    // console.log(4, await bplist.parseBuffer(d.buffer))
}

function rc4Decrypt(e) {
    var r = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : "hjasbdn2ih823rgwudsde7e2dhsdhas";
    "string" == typeof r && (r = [].map.call(r, function(t) {
        return t.charCodeAt(0)
    }));
